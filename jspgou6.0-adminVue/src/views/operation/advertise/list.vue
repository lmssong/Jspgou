<template>
  <div class="forum-module"  v-loading="loading">
        <div class="forum-header">
            <span class="forum-name">广告列表</span>
        </div>
        <div class="table-top-bar clearfix">
            <div class="query-inline-box flex-between">
                <el-button type="primary" @click="routerLink('/advertise/save','save',0)">添加广告</el-button>      
            </div>
        </div>
        <!-- table -->
        <el-table style="width:100%" :data="items"  stripe  @selection-change="checkIds"> 
            ...
            <el-table-column label="广告权重" prop="weight" align="center">   </el-table-column>
            <el-table-column label="点击次数/展现次数" prop="priority" align="center" width="200">
                     <template slot-scope="scope">
                         <span>{{scope.row.clickCount}} / {{scope.row.displayCount}}</span>
                    </template>
            </el-table-column>
            <el-table-column label="是否启用" prop="enabled" align="center">
                      <template slot-scope="scope">
                     <span v-if='!scope.row.enabled' class="red">是</span>
                      <span v-else>否</span>
                    </template>
            </el-table-column>
            <el-table-column label="操作"  align="center" width="200">
                <template slot-scope="scope">
                    <a class="t-edit" title="修改" @click="routerLink('/advertise/update','update',scope.row.id)"></a>
...
data() {
  ...
  methods:{
      getItems() {
      //获取接口数据
      fetch.getAdvertiseList(this.params).then(res => {
    ...
    deleteItems(ids) {
      this.$confirm("确定要删除吗?", "警告", { type: "error" })
        .then(mes => {
          fetch.deleteAdvertiseInfo({ ids: ids }).then(res => {
            if (res.code == 200) {
