<template>
  <div class="forum-module"  v-loading="loading">
        <div class="forum-header">
            <span class="forum-name">api接口调用记录列表</span>
        </div>
        ...
        <el-table style="width:100%" :data="items"  stripe @selection-change="checkIds">
            ...
            <el-table-column label="API接口信息" prop="" align="center"></el-table-column>
            <el-table-column label="调用的账户" prop="appId" align="center"></el-table-column>
            <el-table-column label="调用的IP" prop="apiIp" align="center"></el-table-column>
            <el-table-column label="签名数据" prop="sign" align="center"></el-table-column>
            <el-table-column label="调用时间" prop="apiCallTime" align="center"></el-table-column>
...
data() {
  ...
  methods:{
      getItems() {
      //获取接口数据
      fetch.getApiRecordList(this.params).then(res => {
    ...
    query(){//查询
     ...
     deleteItems(ids) {
      this.$confirm("确定要删除吗?", "警告", { type: "error" })
        .then(mes => {
          fetch.deleteApiRecordInfo({ ids: ids }).then(res => {
            if (res.code == 200) {
