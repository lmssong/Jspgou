<template>
  <div class="forum-module"  v-loading="loading">
        <div class="forum-header">
            <span class="forum-name">在线客服列表</span>
        </div>
        <div class="table-top-bar clearfix">
            <div class="query-inline-box flex-between">
                <el-button type="primary" @click="routerLink('/customerService/save','save',0)">添加客服</el-button>      
            </div>
        </div>
        <!-- table -->
        <el-table style="width:100%" :data="items"  stripe  @selection-change="checkIds"> 
            ...
            <el-table-column label="客服名称" prop="name" align="center"></el-table-column>
            <el-table-column label="客服账号" prop="content" align="center">   </el-table-column>
            ...
            <el-table-column label="操作"  align="center" width="200">
                <template slot-scope="scope">
                    <a class="t-edit" title="修改" @click="routerLink('/customerService/update','update',scope.row.id)"></a>
...
data() {
  ...
  methods:{
      getItems() {
      //获取接口数据
      fetch.getCustomerServiceList(this.params).then(res => {
    ...
    savePriority() {
      ...
      fetch.priorityCustomerServiceInfo(params).then(res => {
        this.loading = false;
        if (res.code == 200) {
    ...
    deleteItems(ids) {
      this.$confirm("确定要删除吗?", "警告", { type: "error" })
        .then(mes => {
          fetch.deleteCustomerServiceInfo({ ids: ids }).then(res => {
            if (res.code == 200) {
