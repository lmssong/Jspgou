<template>
  <div class="forum-module"  v-loading="loading">
        <div class="forum-header">
            <span class="forum-name">配送方式列表</span>
        </div>
        <div class="table-top-bar clearfix">
            <div class="query-inline-box flex-between">
                <el-button type="primary"@click="routerLink('/shipList/save','save',0)" >添加配送方式</el-button>      
            </div>
        </div>
        ...
        <el-table style="width:100%" :data="items"  stripe  @selection-change="checkIds">
           ...
           <el-table-column label="计价方式" prop="priority" align="center">
               <template slot-scope="scope">
                    <span v-if="scope.row.method==1">统一价格：{{scope.row.uniformPrice}}</span>
                     <span v-if="scope.row.method==2">首重价:{{scope.row.firstPrice}},续重价:{{scope.row.additionalPrice}}</span>
                </template> 
            </el-table-column>
            <el-table-column label="排序" prop="priority" align="center">
               ...
               <el-table-column label="是否启用" prop="priority" align="center">
                <template slot-scope="scope">
            ...
            <el-table-column label="操作"  align="center" width="200">
                <template slot-scope="scope">
                    <a class="t-edit" title="修改" @click="routerLink('/shipList/update','update',scope.row.id)"></a>
...
data() {
  ...
  methods:{
      getItems() {
      //获取接口数据
      fetch.getShipList(this.params).then(res => {
    ...
    savePriority() {
      ...
      fetch.priorityShipInfo(params).then(res => {
        this.loading = false;
        if (res.code == 200) {
    ...
    deleteItems(ids) {
      this.$confirm("确定要删除吗?", "警告", { type: "error" })
        .then(mes => {
          fetch.deleteShipInfo({ ids: ids }).then(res => {
            if (res.code == 200) {
