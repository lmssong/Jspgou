    ...
    <div class="forum-header">
           <span class="forum-name">{{$route.name}}</span>
             <div class="pull-right">
                <el-button class="back" @click="routerLink('/discussList','list',0);">返回列表</el-button>
            </div>  
    </div>
    <el-form label-width="160px" class="table-body" ref="form" :model="info" :rules="rules">
      <el-row class="form-group">
        <el-col :xs="24" :sm="24" :md="24" :lg="24">       
         ...
         <el-col :xs="24" :sm="24" :md="24" :lg="24">       
          <el-form-item label="评价回复:" prop="replay" >
            <el-input class="w192" v-model="info.replay" type="textarea"></el-input>
          </el-form-item>
         </el-col>
...
export default {
  ...
  methods: {
    ...
    getInfo(id) {
      fetch.getDiscussInfo({id:id}).then(res => {
        this.info = res.body;
    ...
    updateForm() {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          this.loading = true;
          fetch
            .updateDiscussInfo(this.info)
            .then(res => {
              this.loading = false;
              if (res.code == 200) {
                this.successMessage("回复成功");
                setTimeout(() => {
                    this.routerLink('/discussList','list',0);
                }, 1000);
              }
            })
            .catch(error => {
