  ...
  <div class="forum-module" v-loading='loading'>
    <div class="forum-header">
      ...
      <div class="pull-right">
                <el-button class="back" @click="routerLink('/dictTypeList','list');">返回列表</el-button>
            </div>
    </div>
  ...
  methods: {
    ...
    updateForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          this.loading = true;
          fetch.updateDictTypeInfo(this.info).then(res => {
            this.loading = false;
            if (res.code == 200) {
              this.successMessage("修改成功");
              setTimeout(() => {
                this.routerLink("/dictTypeList", "list");
              }, 1000);
            }
          });
        }
    ...
    saveForm(state) {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          fetch.addDictTypeInfo(this.info).then(res => {
            this.loading = false;
            if (res.code == 200) {
