<template>
  <div class="forum-module"  v-loading="loading">
        <div class="forum-header">
            <span class="forum-name">数据字典列表</span>
        </div>
        <div class="table-top-bar clearfix">
            <div class="query-inline-box flex-between">
                <el-button type="primary"@click="routerLink('/dictList/save','save',0)" >添加数据字典</el-button>      
            </div>
        </div>
        ...
        <el-table style="width:100%" :data="items"  stripe  @selection-change="checkIds">
            ...
            <el-table-column label="字典类型" prop="typeName" align="center"></el-table-column>
            ...
            <el-table-column label="操作"  align="center" width="200">
                <template slot-scope="scope">
                    <a class="t-edit" title="修改" @click="routerLink('/dictList/update','update',scope.row.id)"></a>
...
data() {
  ...
  methods:{
      getItems() {
      //获取接口数据
      fetch.getDictList(this.params).then(res => {
    ...
    savePriority() {
      ...
      fetch.priorityDictInfo(params).then(res => {
        this.loading = false;
        if (res.code == 200) {
    ...
    deleteItems(ids) {
      this.$confirm("确定要删除吗?", "警告", { type: "error" })
        .then(mes => {
          fetch.deleteDictInfo({ ids: ids }).then(res => {
            if (res.code == 200) {
