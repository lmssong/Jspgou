<template>
  <div class="forum-module"  v-loading="loading">
        <div class="forum-header">
           ...
           <span class="forum-name tab-name" @click="routerLink('/wechatset/wechatdefaultMessage','list')">默认回复</span>
              ...
              <div class="pull-right">
                <el-button class="back" @click="getChildList(0)" v-if="params.parentId!=''">返回列表</el-button>
            </div>
        </div>
        <div class="table-top-bar clearfix">
            <div class="query-inline-box flex-between">
                <el-button type="primary"@click="addMenu(params.parentId)" >添加自定义菜单</el-button>      
            </div>
        </div>
        ...
        <el-table style="width:100%" :data="items"  stripe  @selection-change="checkIds">
            ...
            <el-table-column label="操作"  align="center" width="200">
                <template slot-scope="scope">
                    ...
                    <a class="t-order" title="二级菜单管理" @click="getChildList(scope.row.id)" v-if="params.parentId==''"></a>
                    <a class="t-edit" title="修改" @click="routerLink('/wechatset/wechatmenuupdate','update',scope.row.id)"></a>
        ...
        <div class="table-bottom-bar">
            <div class="pull-left">
               <el-button  @click="savePriority">添加到微信菜单</el-button>
...
data() {
  ...
  methods:{
      addMenu(parentId){
          this.$router.push({path:'/wechatset/wechatmenusave',query:{
              type:'save',
              id:0,
              parentId:parentId
          }})
      ...
      getItems() {
      //获取接口数据
      fetch.getWeiXinMenuList(this.params).then(res => {
    ...
    deleteItems(ids) {
      this.$confirm("确定要删除吗?", "警告", { type: "error" })
        .then(mes => {
          fetch.deleteWeiXinMenuInfo({ ids: ids }).then(res => {
            if (res.code == 200) {
