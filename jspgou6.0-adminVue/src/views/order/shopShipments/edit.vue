  ...
  <div class="forum-module" v-loading='loading'>
    <div class="forum-header">
      ...
      <div class="pull-right">
                <el-button class="back" @click="routerLink('/shopShipments','list');">返回列表</el-button>
            </div>
    </div>
    <el-form label-width="160px" class="table-body" ref="form" :model="info" :rules="rules">
      <el-row class="form-group">
        <el-col :xs="24" :sm="24" :md="24" :lg="24">
         ...
         <el-col :xs="24" :sm="24" :md="24" :lg="24">
            <el-col :xs="24" :sm="24" :md="12" :lg="12" class="border-none"></el-col>
          <el-form-item label="发货人联系方式:" prop="name">
       ...
       <el-col :xs="24" :sm="24" :md="24" :lg="24">
          ...
          <el-form-item label="是否默认:" :required="true">
            <el-radio :label="true" v-model="info.isDefault">是</el-radio>
              <el-radio :label="false" v-model="info.isDefault">否</el-radio>
          </el-form-item>
  ...
  methods: {
    ...
    updateForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          this.loading = true;
          fetch.updateShopShipmentsInfo(this.info).then(res => {
            this.loading = false;
            if (res.code == 200) {
              this.successMessage("修改成功");
              setTimeout(() => {
                this.routerLink("/shopShipments", "list");
              }, 1000);
            }
          });
        }
    ...
    saveForm(state) {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          fetch.addShopShipmentsInfo(this.info).then(res => {
            this.loading = false;
            if (res.code == 200) {
