            ...
            <el-row class="form-group dynamic-layout ">
                <el-col class="flex-50">
                    <el-form-item label="上级分类" :required="true">
                       <el-select filterable placeholder="请选择" v-model="parentId"  class="w192">
                            ...
                            <el-option v-for="item in parantList"
                            :key="item.id" 
                ...
                <el-col :class="[model.single ? 'flex-100' :'flex-50']" v-for="(model,index) in modelInfo" :key="model.id" v-if="model.field!='standardTypeIds'">
                    <el-form-item :label="model.propertyName" :prop="model.field" >
                        ...
                        <!-- 商品分类模版 -->
                        <el-select filterable placeholder="请选择" v-model="info.tplChannel" 
                        v-if="model.dataType==6&&model.field=='tplChannel'" class="w192">
                        ...
                        v-model="info.tplContent" v-if="model.dataType==6&&model.field=='tplContent'" class="w192">
                            <el-option label="默认" value=""></el-option>
                        ...
                        <el-input class="w192" v-model="info[model.field]" v-if="model.dataType==1&&model.field!='imagePath'"></el-input>
        ...
        methods: {
            getModelInfo(modelId) {
                ...
                fetch.getModelInfo(params).then(res => {
                    ...
                    this.info['typeId']=modelId;//添加
                    this.info['typeName']=res.body[0].name;//添加
                });
            },
            getInfo(id) { //有多个请求合并在一起发送统一处理
                        this.loading=true;
                axios.all([
                        fetch.getCommodityTopCategory({id:this.parantId}),
                      ...
                      this.$refs['form'].validate((valid) => {
                            if (valid) {
                               ...
                               if(this.checkedStandardType.length>0){
                                ...
                                fetch.addCommodityCategoryInfo(this.info).then(res=>{
                                this.loading=false;
                                if(res.code==200){
                                    this.successMessage('添加成功');
                                        if(state){
                                            this.checkedBrands=[];
                                            this.checkedStandardType=[];
                                            this.imagePath='';               
