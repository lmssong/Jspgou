<template>
  <div class="forum-module"  v-loading="loading">
        <div class="forum-header">
            <span class="forum-name">类型列表</span>
        </div>
        ...
        <el-table style="width:100%" :data="items"  stripe  @selection-change="checkIds">
            ...
            <el-table-column label="商品分类模板前缀" prop="channelPrefix" align="center"></el-table-column>
           <el-table-column label="商品内容模板前缀" prop="contentPrefix" align="center"></el-table-column>
            <el-table-column label="属性列表" prop="priority" align="center" width="300">
                     <template slot-scope="scope">
                         <a href="javascript:void(0)"  @click="modelList(scope.row.id,scope.row.name,true)" class="link-color">[分类字段]</a>
                         <a href="javascript:void(0)"   @click="modelList(scope.row.id,scope.row.name,false)" class="link-color">[商品字段]</a>
                    </template>
            </el-table-column>
            <el-table-column label="操作"  align="center" width="200">
                <template slot-scope="scope">
                    <a class="t-edit" title="修改" @click="routerLink('/typeList/update','update',scope.row.id)"></a>
...
data() {
  ...
  methods:{
      getItems() {
      //获取接口数据
      fetch.getModelList(this.params).then(res => {
    ...
    deleteItems(ids) {
      this.$confirm("确定要删除吗?", "警告", { type: "error" })
        .then(mes => {
          fetch.deleteTypeInfo({ ids: ids }).then(res => {
            if (res.code == 200) {
    ...
    modelList(typeId,name,isCategory){
          if(isCategory){
             this.$router.push({path:'/typeList/modelList',query:{
            typeId:typeId,
              isCategory:isCategory,
             typeName:name
             }})
          }else{
               this.$router.push({path:'/typeList/modelList2',query:{
            typeId:typeId,
              isCategory:isCategory,
             typeName:name
             }})
