<template>
  <div class="forum-module"  v-loading="loading">
        <div class="forum-header">
            <span class="forum-name">品牌列表</span>
        </div>
        <div class="table-top-bar clearfix">
            <div class="query-inline-box flex-between">
                <el-button type="primary" @click="routerLink('/brandList/save','save',0)">添加品牌</el-button>      
            </div>
        </div>
        <!-- table -->
        <el-table style="width:100%" :data="items"  stripe  @selection-change="checkIds"> 
            ...
            <el-table-column label="品牌类型" prop="categoryName" align="center"></el-table-column>
            <el-table-column label="官网网址" prop="webUrl" align="center">
            ...
            <el-table-column label="精选品牌" prop="sift" align="center">
                  <template slot-scope="scope">
                     <span v-if='scope.row.sift' class="red">是</span>
                      <span v-else>否</span>
                    </template>
            </el-table-column>
            ...
            <el-table-column label="操作"  align="center" width="200">
                <template slot-scope="scope">
                    <a class="t-edit" title="修改" @click="routerLink('/brandList/update','update',scope.row.id)"></a>
...
data() {
  ...
  methods:{
      getItems() {
      //获取接口数据
      fetch.getBrandList(this.params).then(res => {
    ...
    savePriority() {
      ...
      fetch.priorityBrandInfo(params).then(res => {
        this.loading = false;
        if (res.code == 200) {
    ...
    deleteItems(ids) {
      this.$confirm("确定要删除吗?", "警告", { type: "error" })
        .then(mes => {
          fetch.deleteBrandInfo({ ids: ids }).then(res => {
            if (res.code == 200) {
